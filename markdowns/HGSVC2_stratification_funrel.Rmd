...
hgsvc2 <- read.csv(paste0(outdir, "/",tag,"_progu120_34_dashing.csv")) %>% 
...
hgsvc2_snv <- read.csv(paste0(outdir, "/",tag,"_progu120_34_dashing.csv")) %>% 
   mutate(dataset="SNVs Only") %>%
  mutate(delta_pos=delta, ngenomes=ngen)

plotProgressiveUnion(species=tag, out = outdir, hgsvc2_snv, nshow=5) + 
  geom_line(data=(summarize(group_by(hgsvc2_snv, ngen, dataset), mean=mean(delta_pos))),
            mapping=aes(y=mean,x=ngen))

plotProgressiveUnion(species="HGSVC2_snv", out = outdir, hgsvc2_snv, nshow=5)
plotProgressiveUnion(species=tag, out = outdir, hgsvc2_snv, nshow=1)
...
plotProgressiveUnion(species=tag, out = outdir, hgsvc2_snv_indel, nshow=5) + 
  geom_line(data=(summarize(group_by(hgsvc2_snv_indel, ngen, dataset), mean=mean(delta_pos))),
            mapping=aes(y=mean,x=ngen))
plotProgressiveUnion(species="HGSVC2_snv_indel", out = outdir, hgsvc2_snv_indel, nshow=5)
plotProgressiveUnion(species="HGSVC2_snv_indel", out = outdir, hgsvc2_snv_indel, nshow=1)
